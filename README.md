...
## Random Stuff To Add To Your `.bashrc`{:.bash} :)
```bash
# -------- C --------
ccr() {
  local file
...
Make sure either the [GNU Compiler Collection](https://www.gnu.org/) or the [LLVM Project](https://llvm.org/) is installed on your computer. This repo  mostly follows the `-std=c17`{:.bash} standard, with some exceptions in the `/projects`{:.bash} folder.
